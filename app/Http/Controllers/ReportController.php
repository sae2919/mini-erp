...
class ReportController extends Controller
{
    ...
    public function accountStatement(Request $request)
    {
        ...
        $payments = $seller->payments()
            ...
            ->map(fn($p) => [
                'date'        => $p->paid_at,
                'type'        => 'payment',
                'description' => "Payment received via ".ucfirst($p->method).($p->reference ? " ({$p->reference})" : ''),
                'debit'       => 0,
                'credit'      => $p->amount,
                'ref'         => $p->reference,
                'id'          => $p->id,
        ...
        $transactions = $dispatches->concat($payments)
            ->sortBy('date')
            ->values();
    ...
    public function sellerPerformance(Request $request)
    {
        ...
        ->map(function($seller) use ($from, $to) {
            ...
            $dispatched  = $seller->dispatchOrders()->whereBetween('dispatch_date', [$from, $to])->sum('total_amount');
            $totalStock  = $seller->stocks()->sum('quantity');
    ...
    public function stockMovement(Request $request)
    {
        ...
        $products = Product::active()->with('category')
            ...
            ->map(function($product) use ($from, $to) {
                $produced  = ProductionItem::whereHas('production',
            ...
            ->filter(fn($p) => $p['produced'] > 0 || $p['dispatched'] > 0 || $p['sold'] > 0 || $p['total_stock'] > 0)
            ->values();
