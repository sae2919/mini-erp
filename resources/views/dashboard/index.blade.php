    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <p class="text-2xl font-bold text-gray-900 mt-1">
                {{ number_format($warehouseStock ?? 0) }} units
            </p>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <p class="text-2xl font-bold text-blue-700 mt-1">
                ₹{{ number_format($totalDispatched ?? 0, 0) }}
            </p>
            <p class="text-xs text-gray-400 mt-1">
                This month: ₹{{ number_format($dispatchThisMonth ?? 0, 0) }}
            </p>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <p class="text-2xl font-bold text-purple-600 mt-1">
                ₹{{ number_format($pendingCommissions ?? 0, 0) }}
            </p>
            <p class="text-xs text-gray-400 mt-1">
                Total: ₹{{ number_format($totalCommissions ?? 0, 0) }}
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-yellow-50 border border-yellow-100 rounded-xl p-4">
            ...
            <p class="text-xl font-bold text-yellow-800 mt-1">
                ₹{{ number_format($productionThisMonth ?? 0, 0) }}
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <h2 class="font-semibold text-gray-800 mb-4">
                📊 Monthly — Dispatched vs Collected vs Sold
            </h2>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm">
            ...
            <div class="flex items-center justify-between px-5 py-3 border-b border-gray-50 last:border-0">
                <div>
                    ...
                    <p class="text-xs text-gray-400">{{ $s->sales_count ?? 0 }} sales</p>
                </div>
            ...
            <div class="flex items-center justify-between px-5 py-3 border-b border-gray-50">
                <div>
                    ...
                    <p class="text-xs text-gray-400">
                        {{-- FIX: null-safe on seller in case it was soft-deleted --}}
                        {{ $d->seller?->name ?? '—' }}
                        ·
                        {{-- FIX: use Carbon::parse() instead of assuming Carbon cast --}}
                        {{ \Carbon\Carbon::parse($d->dispatch_date)->format('d M') }}
                    </p>
                </div>
                <div class="text-right">
                    ...
                    {{-- FIX: guard against missing paymentStatusColor() method --}}
                    @if(method_exists($d, 'paymentStatusColor'))
                    <span class="text-xs px-1.5 py-0.5 rounded-full {{ $d->paymentStatusColor() }}">
                        {{ ucfirst($d->payment_status ?? '') }}
                    </span>
                    @else
                    <span class="text-xs text-gray-400">
                        {{ ucfirst($d->payment_status ?? 'unknown') }}
                    </span>
                    @endif
                </div>
            </div>
...
<script>
@php
    // FIX: guard against empty or missing $monthlyData
    $chartData = collect($monthlyData ?? []);
@endphp
...
new Chart(document.getElementById('monthlyChart'), {
    ...
    data: {
        ...
        datasets: [
            {
                label:           'Dispatched',
                data:            @json($chartData->pluck('dispatched')),
                backgroundColor: 'rgba(99,102,241,0.75)',
                borderRadius:    4,
            },
            {
                label:           'Collected',
                data:            @json($chartData->pluck('collected')),
                backgroundColor: 'rgba(34,197,94,0.75)',
                borderRadius:    4,
            },
            {
                label:           'Sold',
                data:            @json($chartData->pluck('sold')),
